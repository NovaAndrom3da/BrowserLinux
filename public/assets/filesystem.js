...
window.get_filesystem_object = function(f) {
  f = f.split('/');
  if (f[f.length] - 1 == '') {
    f.pop()
  }
...
window.FILESYSTEM_READ = function(f) {
  return get_filesystem_object(f).file.toString();
}

window.FILESYSTEM_WRITE = function(f, v) {
  get_filesystem_object(f).file = v;
}

window.FILESYSTEM_LIST = function(d) {
  return Object.keys(get_filesystem_object(d));
}

window.FILESYSTEM_FILE = function(d, n) {
  get_filesystem_object(d)[n] = { file: '' };
  return get_filesystem_object(d)[n]
}